					...
					if ( winner >= 0 || iteration < 0 ) {
						...
						for ( i = 0; i < units.size(); i++ ) {
							...
							if ( unit[ UNIT_PROP_TARGET_UNIT ] >= 0 ) {
								...
								if ( (x-x2)*(x-x2) + (y-y2)*(y-y2) < unit[ UNIT_PROP_FIRE_RANGE ] * unit[ UNIT_PROP_FIRE_RANGE ] ) {
									...
									unit[ UNIT_PROP_FACING_THETA ] = (int) ( Math.atan2( x2 - x, y2 - y ) / -MATH_PI * 180 + 90 );
								}
								else {
							...
							if ( unit[ UNIT_PROP_TARGET_X ] >= 0 ) {
								...
								if ( p*p + q*q < unit[ UNIT_PROP_V ] * unit[ UNIT_PROP_V ] ) {
									...
									tries:
									for ( l = 0; l < 4; l++ ) {
										theta = Math.atan2( p, q ) + ( l == 1 ? MATH_HALF_PI : l == 2 ? -MATH_HALF_PI : l == 3 ? MATH_PI : 0 );
										x2 = unit[ UNIT_PROP_X ] += unit[ UNIT_PROP_V ] * Math.sin( theta );
										y2 = unit[ UNIT_PROP_Y ] += unit[ UNIT_PROP_V ] * Math.cos( theta );
						...
						for ( i = bullets.size() - 1; i >= 0 ; i-- ) { // Descending order is a must because bullets might get removed here
							...
							if ( p*p + q*q < 4 * unit2[ UNIT_PROP_V ] * unit2[ UNIT_PROP_V ] ) {
								...
								bullet[ BULLET_PROP_X ] += 2 * unit2[ UNIT_PROP_V ] * Math.sin( theta );
								bullet[ BULLET_PROP_Y ] += 2 * unit2[ UNIT_PROP_V ] * Math.cos( theta );
							}
						}
					...
					for ( i = 0; i < units.size(); i++ ) {
						...
						if ( unit[ UNIT_PROP_HEALTH ] < 0 ) {
							...
							for ( p = 0; p < s*3; p++ ) {
								...
								graphics.fillOval( (int) ( x + r * Math.cos( theta ) ), (int) ( y + r * Math.sin( theta ) ), 3, 3 );
							}
							// Explosion
